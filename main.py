...
DEMOGRAPHICS = {
    'study_id':{'type':'string','coerce':str,'regex':'^ww|cc|la|hb\d{5}\w','maxlength':8, 'empty':False}, 
    'age':{'type':'integer','coerce':int,'min':18,'max':89,'empty':False},'country_birth':{'type':'string','coerce':str,'empty':True},
    'employment':{'type':'string','coerce':str,'allowed':['Employed full time','Employed part time','Self employed','Unemployed','Student','Military','Retired','Unknown'], 'empty':True},
    'ethnicity':{'type':'string','coerce':str,'allowed':['Hispanic','Not Hispanic','Unknown'],'empty':False},
    'gi_uds':{'type':'string','coerce':str,'allowed':['Male','Female','Transgender Man/Transgender Male/Transgender Masculine','Transgender Woman/Transgender Female/Transgender Feminine','Other','Choose not to disclose'],'empty':False},
    'household_num':{'type':'integer','coerce':int,'min':1,'max':15,'empty':True},
    'housing_status':{'type':'string','coerce':str,'allowed':['Permanent','Temporary','Unstable','Unknown'],'empty':True},
    'income':{'type':'string','coerce':str,'allowed':['Less than $10,000','$10,000 to $39,999','$40,000 to $69,999','$70,000 to $99,999','$100,000 or more'],'empty':True},
    'language':{'type':'string','coerce':str,'empty':True},
    'race_count':{'type':'integer','coerce':int,'min':1,'max':9,'empty':False},
    'sexual_orientation':{'type':'string','coerce':str,'allowed':['Lesbian or Gay','Heterosexual (or straight)','Bisexual','Something else',"Don't know",'Choose not to disclose'],'empty':False},
    'zip_code_stem':{'type':'string','coerce':str,'maxlength':3,'regex':'\d{3}','empty':False}}

def validation(data, context):
    BUCKET_NAME = data['bucket']
    FILE_NAME = data['name']
    GCS_CSV_PATH = f'gs://demographics_raw_data/{FILE_NAME}'
    GCS_JSON_PATH = f'gs://demographics_raw_data/{FILE_NAME}.json'
    ...
    if _check_validation(DEMOGRAPHICS, dataf):
